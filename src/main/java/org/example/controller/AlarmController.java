...
public class AlarmController {
    ...
    public SseEmitter diagnoseAlarm(@RequestBody Map<String, String> request) {
        ...
        logger.info("启动告警诊断: taskId={}, description={}, graphEnabled={}", taskId, alarmDescription, graphEnabled);
        ...
        diagnosisExecutor.execute(() -> {
            try {
                ...
                if (graphEnabled && compiledGraph != null) {
                    result = invokeGraphForDiagnosis(diagnosisInput, taskId);
                } else {
                    result = invokeLegacyDiagnosis(diagnosisInput);
                }
    ...
    private String invokeGraphForDiagnosis(String diagnosisInput, String taskId) {
        try {
            ...
            logger.warn("Graph引擎告警诊断失败，降级到旧编排: {}", e.getMessage());
            return invokeLegacyDiagnosis(diagnosisInput);
        }
    }
    ...
    public Map<String, Object> resumeFromCheckpoint(@PathVariable String taskId) {
        ...
        String result = agentOrchestrationService.resumeFromCheckpoint(taskId);
